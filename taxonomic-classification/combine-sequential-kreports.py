#!/usr/bin/env python
# ============================================================================
# Combination of multiple kraken-style reports
# ============================================================================
# Based on the script in KrakenTools by Jennifer Lu
#
# Edited to correctly combine sequential reports as done in our workflow
# .:. The number of unclassified reads will be the lowest and not the sum
#
...
def main():
    ...
    prev_node = -1
    curr_node = -1
    #Edited: Initialize u_reads with infinity for tracking the minimum value
    u_reads = {0: float('inf')}
    total_reads = {0:0} 
    taxid2node = {}
    ...
    for r_file in args.r_files:
        ...
        for line in curr_file: 
            ...
            #Total reads calculation # Edit to account for new way to calculate unclassified reads
            if level_id == 'U' or taxid == '0':
                # Update min_unclassified_reads with the minimum value for unclassified reads
                min_unclassified_reads = min(float('inf'), level_reads)
            else:
                total_reads[0] += level_reads
                total_reads[count_samples] = level_reads
            #Unclassified #Edit: Update the unclassified reads handling in the loop to track the minimum value:
            if level_id == 'U' or taxid == '0':
                u_reads[0] = min(u_reads[0], level_reads)
            ...
            prev_node = curr_node 
        curr_file.close()
    total_reads[0] += min_unclassified_reads
    ...
    # Calculate the denominator for the percentage calculation
    denominator = total_reads[0]
    # Calculate the percentage of unclassified reads
    percentage_unclassified = (float(u_reads[0]) / denominator) * 100
    # Write the percentage to the output file
    o_file.write("%0.4f\t" % percentage_unclassified)
    for i in u_reads:
        #edit: Print the unclassified reads with the minimum value in the final report:
        if i == 0 or (i > 0 and not args.c_only):
            if i not in u_reads:
                o_file.write("0\t")
                o_file.write("0\t")
            else:
                o_file.write("%i\t" % u_reads[0])
                o_file.write("%i\t" % u_reads[0])
